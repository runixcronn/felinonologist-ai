...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // Attempt to insert into Users table as requested
        // Using likely column names based on description
        const { error: dbError } = await supabase.from('Users').insert({
          uid: data.user.id,
          display_name: fullName,
          email: email,
          phone: null, // Phone not collected in this form yet
          providers: ['email'],
          provider_type: 'email',
          created_at: new Date().toISOString(),
          last_sign_in_at: new Date().toISOString() // Assuming signed in or just created
        });

        if (dbError) {
          console.error("Error creating user record:", dbError);
          // We continue even if DB insert fails (might be handled by trigger or RLS)
        }
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted mt-2 text-sm">
            Kedinizin en iyi dostu olmak için ücretsiz hesap oluşturun
          </p>
        </div>
        ...
        <form ref={formRef} onSubmit={handleRegister} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted group-focus-within:text-orange-500 transition-colors" />
              <input 
                type="text" 
                value={fullName}
          ...
          <div className="flex items-start gap-3 mt-2">
             <div className="relative flex items-center h-5">
               <input
                 type="checkbox"
                 required
                 className="w-4 h-4 rounded border-border text-orange-500 focus:ring-orange-500/20 bg-surface-elevated/50"
               />
             </div>
             <p className="text-xs text-muted leading-relaxed">
               <Link href="/terms" className="text-orange-500 hover:underline">Kullanım Koşulları</Link>'nı ve <Link href="/privacy" className="text-orange-500 hover:underline">Gizlilik Politikası</Link>'nı kabul ediyorum.
             </p>
          </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-surface px-2 text-muted">Hesabınız var mı?</span>
            </div>
          </div>
